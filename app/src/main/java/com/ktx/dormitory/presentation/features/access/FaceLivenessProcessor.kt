...
class FaceLivenessProcessor {
    private val _state = MutableStateFlow(FaceLivenessUiState())
    ...
    fun process(face: Face) {
        ...
        when (_state.value.currentStep) {
            ...
            LivenessStep.TURN_LEFT -> {
                if (face.headEulerAngleY > 25f) { // Quay trái
            ...
            LivenessStep.TURN_RIGHT -> {
                if (face.headEulerAngleY < -25f) { // Quay phải
    ...
    private fun moveToNextStep(next: LivenessStep) {
        _state.update { it.copy(currentStep = next, progress = calculateProgress(next)) }
    }
